...
#import "NSObject+CJGChain.h"
#import <UIKit/UIKit.h>
#import <objc/message.h>
@implementation NSObject (CJGChain)
...
#define CJGPERFORMSELECROR(_args, _return_)\
...
static const void *CJGRuntimeDeallocTasks = &CJGRuntimeDeallocTasks;
static const void *CJGRuntimeDeallocClassTag = &CJGRuntimeDeallocClassTag;


static inline void CJG_swizzleDeallocIfNeed(Class swizzleClass){
    @synchronized (swizzleClass) {
        if (objc_getAssociatedObject(swizzleClass, CJGRuntimeDeallocClassTag)) return;
        objc_setAssociatedObject(swizzleClass, CJGRuntimeDeallocClassTag, @1, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        ...
        id newImpBlock = ^ (__unsafe_unretained id self){
            ...
            NSMutableArray *deallocTask = objc_getAssociatedObject(self, CJGRuntimeDeallocTasks);
            @synchronized (deallocTask) {
                if (deallocTask.count > 0) {
                    [deallocTask enumerateObjectsUsingBlock:^(CJG_deallocTask obj, NSUInteger idx, BOOL * _Nonnull stop) {
                        if (obj) {
                            obj(self);
                        }
                    }];
...
//dealloc
- (NSMutableArray<CJG_deallocTask> *)CJG_deallocTasks{
    NSMutableArray *tasks = objc_getAssociatedObject(self, CJGRuntimeDeallocTasks);
    if (tasks) return tasks;
    tasks = [NSMutableArray array];
    CJG_swizzleDeallocIfNeed(object_getClass(self));
    objc_setAssociatedObject(self, CJGRuntimeDeallocTasks, tasks, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    return tasks;
}

- (void)addDeallocTask:(void (^)(id _Nonnull))task{
    @synchronized ([self CJG_deallocTasks]) {
        [[self CJG_deallocTasks] addObject:task];
    }
}
